...
/*group 1
group members:
Farhana Alam, 2018-1-60-025
Ishrat Jaben Bushra, 2018-1-60-099
Khairum Islam, 2018-1-60-265*/
...
{ 
	...
	{ 
        ...
        printf("\nStudent %d received help.\n",s_id);
 
		sem_wait(&mutex);
        priority[s_id-1]++;
		printf("\nAfter getting help, Student %d priority now is %d\n",s_id, priority[s_id-1]);
		sem_post(&mutex);
    }
}
...
void *tutor_thread(void *tutor_id)
{
    ...
    while(1)
	{
        ...
        int max_request=999, max_priority = help-1 ,s_id = -1;
		sem_wait(&mutex);
    ...
    while(1)
	{
        ...
        for(int i=0;i<student_no;i++)
		{
            if(visited[i]>-1)
			{
                ...
                printf("\nCoordinator: Student %d has priority %d.\n",student_ids[i],priority[i]);
                visited[i]=-1;
